...
if (isset($_GET['id']) && isset($_POST['url'])) {
    $id_imunisasi = dekripsi($_GET['id']);

    $data = query("SELECT * FROM imunisasi WHERE id_imunisasi = $id_imunisasi")[0];
    $id_balita = $data['id_balita'];

    $balita = query("SELECT * FROM balita WHERE id_balita = $id_balita")[0];

    $idposyandu = $balita['idposyandu'];
    $idjadwal = $data['idjadwal'];

    $jadwal = query("SELECT * FROM jdw_posyandu WHERE jadwal > DATE_SUB(NOW(), INTERVAL 24 HOUR) AND idposyandu = $idposyandu AND idjadwal NOT IN (SELECT idjadwal FROM imunisasi WHERE id_balita = $id_balita) ORDER BY jadwal ASC");
...
<body>
    ...
    <div class="main-container m-0">
        <div class="d-flex">
            ...
            <div class="contents">
                <h4 class="text-center fw-bold" style="color: black;">MANAJEMEN DATA IMUNISASI</h4>
                ...
                <form method="post" action="">
                    ...
                    <div class="mb-3 mt-5 row ms-5">
                        ...
                        <div class="col-sm-6">
                            <input type="text" class="form-control" style="border: 1px solid black;" id="balita"
                                value="<?= $balita['nama_balita']; ?>" disabled>
                        </div>
                    </div>
                        ...
                        <div class="col-sm-6">
                            <select class="boxc form-control" style="border-color: black;" name="idjadwal" require
                                ...
                                foreach ($jadwal as $j):
                                    ...
                                    ?>
                                    <option value="<?= $j['idjadwal'] ?>" <?= $j['idjadwal'] == $data['idjadwal'] ? 'selected' : '' ?>><?= $jadwal; ?> di Posyandu <?= $nama_posyandu['nama_posyandu']; ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="mb-3 mt-2 row ms-5">
                        <label for="jenis" class="col-sm-3 me-0 col-form-label">JENIS IMUNISASI</label>
                        <div class="col-sm-6">
                            <select class="boxc form-control" style="border-color: black;" name="jenis_imunisasi"
                                require id="jenis">
                                <option hidden selected value="">--Pilih Jenis Imunisasi--</option>
                                <?php foreach ($imunisasi_array as $imun): ?>
                                    <option value="<?= $imun ?>" <?= $imun == $data['jenis_imunisasi'] ? "selected" : ""; ?>>
                                        <?= $imun; ?></option>
                                <?php endforeach ?>
                            </select>
                        </div>
...
if (isset($_POST['submit'])) {
    if (update($_POST) > 0) {
        $_SESSION["berhasil"] = "Data Imunisasi Berhasil Diubah!";
        echo "
            <script>
